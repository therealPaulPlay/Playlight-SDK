	...
	<button
		...
		class="fixed {position == 'bottom-left'
			...
			: position == 'bottom-right'
				? 'right-4 bottom-4'
				: ''} group children-text-white text-black z-99990 flex cursor-pointer gap-2 rounded-md bg-white p-3 font-bold uppercase shadow-lg transition-all hover:scale-105"
		onclick={() => {
			onClick?.();
		}}
