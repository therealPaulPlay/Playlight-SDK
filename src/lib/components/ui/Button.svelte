<script>
	let { onclick, children, class: extraClasses, disabled = false, variant = "primary" } = $props();
</script>
...
<button
	class="{variant == 'primary'
		...
		: ''} font-bold text-nowrap uppercase transition hover:text-white {disabled ? "pointer-events-none opacity-50" : ""} {extraClasses}"
	{onclick}
>
	{@render children?.()}
</button>
