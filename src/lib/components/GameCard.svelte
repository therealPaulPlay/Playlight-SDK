...
<div
	class="bg-muted highlight-border relative mt-5 mb-[calc(3dvh+1.5vw)] flex aspect-[2/3] h-1/2 max-h-[75vh] min-h-92 cursor-pointer flex-col border-1 shadow-xl transition hover:brightness-105 lg:h-3/7"
	...
	onclick={handleClick}
>
	<div class="bg-background aspect-[2/3] h-full overflow-hidden">
		<!-- Cover Video -->
		...
		{#if isHovered}
			...
			>
				<div class="hide-scrollbar h-full w-full overflow-y-auto">
					<h3 class="mb-2 truncate text-lg text-center font-bold">{game.name}</h3>
					<p class="text-muted-foreground mb-4 text-sm text-center text-balance">{game.description || "No description."}</p>
				</div>
